if (state == "INIT") {
		...
		//Spawn party
		partysize = array_length(ga_party1);
		heroX = 4;
		heroY = 180;
	
		for (var i = 0; i < partysize; i++){
			hero = instance_create_depth(heroX + (i*32), heroY, -100, obj_BOSShero);
			hero.index = i;
			hero.dead = false;
			hero.stunned = 0;
			hero.isasleep = false;
		...
		totalmonstergroups = MAX_MONSTER_GROUPS;
			...
			monstergroup = instance_create_depth(a_monster_pos[0, 0], a_monster_pos[0, 1], -100, obj_EggBoss1)	
			monstergroup.number = MAX_MONSTERS_PER_GROUP;
			monstergroup.dead = false;
			monstergroup.hp = ga_bosses[0, 1];
	...
	if !audio_is_playing(mus_Edgar_Finch)
	{
	audio_play_sound(mus_Edgar_Finch, 1, true, 50);
	}
...
if (state == "READY"){
	if playerturn{
		if actionstate == "INIT"{
			...
			with (obj_BOSShero){
				if (ga_party1[index, 2] > 0) ds_list_add(other.ds_heroes, id);
				isdefending = false;
			}
			...
			if (keyboard_check_pressed(vk_space)){
					...
					if menuselected == 0{
						if optionstate != "ATTACK"{
							...
							heromaxdamage = ga_party1[herotocommand.index, 5];
							herototaldamage = irandom_range(8, heromaxdamage);
							
							scr_eggbossdamage(herototaldamage, optiontarget);
					...
					if menuselected == 1{
						if (optionstate != "PSI"){
							...
							for (var spells = 6; spells < 10; spells++){
								if (ga_party1[herotocommand.index, spells] == true){
							...
							if (ds_list_size(ds_psi) > 0) && (ga_party1[herotocommand.index, 4] >= 15)  psistate = "CHOOSE";
							else audio_play_sound(snd_E_B, 1, false);
							
						}else{
							//Cast spell
							if (ds_psi[| selectedspell] <= 7){
								if (ds_psi[| selectedspell] == 6) herototaldamage = irandom_range(-4, -8);
								else if (ds_psi[| selectedspell] == 7) herototaldamage = irandom_range(4, 8);
								
								ga_party1[herotocommand.index, 4] -= 5;
								scr_eggbossdamage(herototaldamage, optiontarget);
								...
								if (ds_psi[| selectedspell] == 8){
									...
									audio_play_sound(snd_psyche,1,false);
									ga_party1[herotocommand.index, 4] -= 10;
								}
								//SLEEP
								if (ds_psi[| selectedspell] == 9){
									...
									audio_play_sound(snd_psyche,1,false);
									ga_party1[herotocommand.index, 4] -= 15;
								}
							}
				...
				if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
					...
					if optiontarget.sprite_index == spr_btl_WeakEdgar{
	...
	if (!playerturn){
		...
		if actionstate == "READY"{
			...
			if (attacktimer >= timetillmonsterattacks){
				...
				//Pick A Victim >:}
				victim = ds_target_heroes[| 0];
				maxmonsterdmg = ga_bosses[0, 5];
				damage = irandom_range(5, maxmonsterdmg);
				
				scr_eggbossdamage(damage, victim);
				}
				attacktimer = 0;
			}
...
if state == "BATTLE OVER"{
	...
	{
		audio_stop_sound(mus_Edgar_Finch);	
