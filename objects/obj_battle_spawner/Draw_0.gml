...
numberofheroes = array_length(ga_party2);
uiX = BUFFER;
uiY = room_height - (numberofheroes * (fontsize + BUFFER));

for (var i = 0; i < numberofheroes; i++){
	draw_set_font(FontNumbers);
	draw_text(uiX + string_width("H") + (i*32), uiY-32, string(ga_party2[i, 2]) + string("HP"));
	draw_text(uiX + string_width("H") + (i*32), uiY-24, string(ga_party2[i, 4]) + string(" PP"));
}
