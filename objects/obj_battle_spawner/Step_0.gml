if (state == "INIT") {
	...
	ds_target_monsters = ds_list_create();
	
	with (obj_hero){
		instance_destroy();	
	}
	
	with (obj_monster_group){
		instance_destroy();	
	}
	//Spawn party
	partysize = array_length(ga_party2);
	heroX = 4;
	heroY = 180;
	
	for (var i = 0; i < partysize; i++){
		hero = instance_create_depth(heroX + (i*32), heroY, -100, obj_hero);
		hero.index = i;
		hero.dead = false;
		hero.stunned = 0;
		hero.isasleep = false;
	...
	for(var i = 0; i < totalmonstergroups; i++){
		monstergroup = instance_create_depth(a_monster_pos[i, 0], a_monster_pos[i, 1], -100, obj_monster_group)	
		monstergroup.type = irandom_range(0,3);
		monstergroup.number = MAX_MONSTERS_PER_GROUP;
		monstergroup.dead = false;
		monstergroup.hp = ga_enemies[monstergroup.type, 1];
		monstergroup.stunned = 0;
		monstergroup.isasleep = false;
		ds_list_add(ds_target_monsters, monstergroup);
	}
	playerturn = true;
	actionstate = "INIT";
	psistate = "MAIN";
...
if (state == "READY"){
	...
	if !audio_is_playing(mus_Battle_Against_A_Weak_Enemy)
	{
	audio_play_sound(mus_Battle_Against_A_Weak_Enemy, 1, true, 50);
	}
	...
	if playerturn{
		if actionstate == "INIT"{
			...
			with (obj_hero){
				if (ga_party2[index, 2] > 0) ds_list_add(other.ds_heroes, id);
				isdefending = false;
			}
			...
			if (keyboard_check_pressed(vk_space)){
					...
					if menuselected == 0{
						if optionstate != "ATTACK"{
							...
							selectedactor = ds_list_find_index(ds_target_monsters, optiontarget)
							...
							heromaxdamage = ga_party2[herotocommand.index, 5];
							herototaldamage = irandom_range(1, heromaxdamage);
							
							scr_damage(herototaldamage, optiontarget);
					...
					if menuselected == 1{
						if (optionstate != "PSI"){
							...
							for (var spells = 6; spells < 10; spells++){
								if (ga_party2[herotocommand.index, spells] == true){
							...
							if (ds_list_size(ds_psi) > 0) && (ga_party2[herotocommand.index, 4] >= 15)  psistate = "CHOOSE";
							...
							if (ds_psi[| selectedspell] <= 7){
								...
								ga_party2[herotocommand.index, 4] -= 5;
								...
								if (ds_psi[| selectedspell] == 8){
									...
									ga_party2[herotocommand.index, 4] -= 10;
									audio_play_sound(snd_psyche,1,false);
								}
								//SLEEP
								if (ds_psi[| selectedspell] == 9){
									...
									ga_party2[herotocommand.index, 4] -= 15;
									audio_play_sound(snd_psyche,1,false);
								}
							}
				...
				if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
					...
					if optiontarget.sprite_index == spr_monsters{
						...
						if (keyboard_check_pressed(global.key_down)){
							if ((selectedactor + 1) < ds_list_size(ds_target_monsters)){
								selectedactor++;
							}else{
								selectedactor = 0;	
							}
							optiontarget = ds_target_monsters[| selectedactor];
						}
						if keyboard_check_pressed(global.key_up){
							if ((selectedactor - 1) >= 0){
								selectedactor--;	
							}else{
								selectedactor = (ds_list_size(ds_target_monsters) - 1);	
							}
							optiontarget = ds_target_monsters[| selectedactor];
						}
					}else{
	...
	if (!playerturn){
		if actionstate == "INIT"{
			...
			//Add valid heroes to list
			with(obj_monster_group){
				if number > 0 ds_list_add(other.ds_monsters, id);	
			}
			
			ds_list_sort(ds_monsters, true);
		...
		if actionstate == "READY"{
			...
			if (attacktimer >= timetillmonsterattacks){
				...
				activemonster = ds_list_find_value(ds_monsters, 0);
				ds_list_delete(ds_monsters, 0);
				...
				maxmonsterdmg = ga_enemies[activemonster.image_index, 5];
				damage = irandom_range(1, maxmonsterdmg);
				
				scr_damage(damage, victim);
				}
				attacktimer = 0;
			}
			
			if ds_list_size(ds_monsters) <= 0{
				actionstate = "INIT";
				playerturn = true;
			}
		}
...
if state == "BATTLE OVER"{
	...
	audio_stop_sound(mus_Battle_Against_A_Weak_Enemy);
