...
for(var i = 0; i < array_length(a_menu); i++;){
	...
	if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
		draw_sprite_ext(spr_btlarrow, 0, optiontarget.x + (sprite_get_width(spr_mchmonsters) / 8), optiontarget.y-36, 1, 1, 270, c_white, 1);	
...
numberofheroes = array_length(ga_party)
uiX = BUFFER;
uiY = room_height - (numberofheroes * (fontsize + BUFFER));

for (var i = 0; i < numberofheroes; i++){
	draw_set_font(FontNumbers);
	draw_text(uiX + string_width("H") + (i*32), uiY-8, string(ga_party[i, 2]) + string("HP"));
	draw_text(uiX + string_width("H") + (i*32), uiY, string(ga_party[i, 4]) + string(" PP"));
}
...
if state == "BATTLE OVER"{
	...
	draw_set_alpha(1);
	draw_text(room_width/2, room_height/2, string(battleovertext));
}