if (state == "INIT") {
	...
	ds_target_monstersMCH = ds_list_create();
	
	with (obj_heroMCH){
		instance_destroy();	
	}
	
	with (obj_monster_groupMCH){
		instance_destroy();	
	}
	//Spawn party
	...
	for (var i = 0; i < partysize; i++){
		hero = instance_create_depth(heroX + (i*32), heroY, -100, obj_heroMCH);
		hero.index = i;
		hero.dead = false;
		hero.stunned = 0;
		hero.isasleep = false;
	...
	for(var i = 0; i < totalmonstergroups; i++){
		monstergroup = instance_create_depth(a_monster_pos[i, 0], a_monster_pos[i, 1], -100, obj_monster_groupMCH)	
		monstergroup.type = irandom_range(0,2);
		monstergroup.number = MAX_MONSTERS_PER_GROUP;
		monstergroup.dead = false;
		monstergroup.hp = ga_MCHenemies[monstergroup.type, 1];
		monstergroup.stunned = 0;
		monstergroup.isasleep = false;
		ds_list_add(ds_target_monstersMCH, monstergroup);
	}
	playerturn = true;
	actionstate = "INIT";
	psistate = "MAIN";
...
if (state == "READY"){
	if !audio_is_playing(mus_Battle_Against_A_Machine)
	{
	audio_play_sound(mus_Battle_Against_A_Machine, 1, true, 50);
	}
			...
			if (keyboard_check_pressed(vk_space)){
					...
					if menuselected == 0{
						if optionstate != "ATTACK"{
							...
							selectedactor = ds_list_find_index(ds_target_monstersMCH, optiontarget)
							...
							scr_damageMCH(herototaldamage, optiontarget);
				...
				if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
					...
					if optiontarget.sprite_index == spr_mchmonsters{
						...
						if (keyboard_check_pressed(global.key_down)){
							if ((selectedactor + 1) < ds_list_size(ds_target_monstersMCH)){
								selectedactor++;
							}else{
								selectedactor = 0;	
							}
							optiontarget = ds_target_monstersMCH[| selectedactor];
						}
						if keyboard_check_pressed(global.key_up){
							if ((selectedactor - 1) >= 0){
								selectedactor--;	
							}else{
								selectedactor = (ds_list_size(ds_target_monstersMCH) - 1);	
							}
							optiontarget = ds_target_monstersMCH[| selectedactor];
						}
					}else{
	...
	if (!playerturn){
		if actionstate == "INIT"{
			...
			//Recreate list
			ds_monstersMCH = ds_list_create();
			
			//Add valid heroes to list
			with(obj_monster_groupMCH){
				if number > 0 ds_list_add(other.ds_monstersMCH, id);	
			}
			
			ds_list_sort(ds_monstersMCH, true);
		...
		if actionstate == "READY"{
			...
			if (attacktimer >= timetillmonsterattacks){
				...
				activemonster = ds_list_find_value(ds_monstersMCH, 0);
				ds_list_delete(ds_monstersMCH, 0);
				...
				if ga_MCHenemies[activemonster.image_index, 4] > 15
				{
					roll = choose("ATT", "PSI")
					
					if roll == "PSI" && ga_MCHenemies[activemonster.image_index, 10] == true 
					{
					...
					scr_damageMCH(-25, activemonster);
					}
				...
				randomvictim = irandom(ds_list_size(ds_target_heroes) - 1);
				victim = ds_target_heroes[| randomvictim];
				maxmonsterdmg = ga_MCHenemies[activemonster.image_index, 5];
				damage = irandom_range(maxmonsterdmg-10, maxmonsterdmg);
				
				scr_damageMCH(damage, victim);	
			...
			if ds_list_size(ds_monstersMCH) <= 0{
				actionstate = "INIT";
				playerturn = true;
			}
		}
...
if state == "BATTLE OVER"{
	...
	audio_stop_sound(mus_Battle_Against_A_Machine);
