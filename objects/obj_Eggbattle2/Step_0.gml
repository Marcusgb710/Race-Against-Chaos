if (state == "INIT") {
		...
		//Spawn party
		partysize = array_length(ga_party3);
		heroX = 4;
		heroY = 180;
	
		for (var i = 0; i < partysize; i++){
			hero = instance_create_depth(heroX + (i*32), heroY, -100, obj_BOSShero2);
			hero.index = i;
			hero.dead = false;
			hero.stunned = 0;
			hero.isasleep = false;
		...
		//Spawn Monsters
		totalmonstergroups = MAX_MONSTER_GROUPS;
	
			monstergroup = instance_create_depth(a_monster_pos[0, 0], a_monster_pos[0, 1], -100, obj_EggBoss2)	
			monstergroup.number = MAX_MONSTERS_PER_GROUP;
			monstergroup.dead = false;
			monstergroup.hp = ga_bosses[1, 1];
			monstergroup.stunned = 0;
			monstergroup.isasleep = false;
			ds_list_add(ds_target_boss, monstergroup);
	...
	if !audio_is_playing(mus_Edgar_Means_Business)
	{
	audio_play_sound(mus_Edgar_Means_Business, 1, true, 50);
	}
...
if (state == "READY"){
	if playerturn{
		if actionstate == "INIT"{
			...
			with (obj_BOSShero2){
				if (ga_party3[index, 2] > 0) ds_list_add(other.ds_heroes, id);
				isdefending = false;
			}
			...
			if (keyboard_check_pressed(vk_space)){
					...
					if menuselected == 0{
						if optionstate != "ATTACK"{
							...
							selectedactor = ds_list_find_index(ds_target_boss, optiontarget)
							...
							heromaxdamage = ga_party3[herotocommand.index, 5];
							herototaldamage = irandom_range(8, heromaxdamage);
							
							scr_eggbossdamage2(herototaldamage, optiontarget);
					...
					if menuselected == 1{
						if (optionstate != "PSI"){
							...
							for (var spells = 6; spells < 10; spells++){
								if (ga_party3[herotocommand.index, spells] == true){
							...
							if (ds_list_size(ds_psi) > 0) && (ga_party3[herotocommand.index, 4] >= 15)  psistate = "CHOOSE";
							else audio_play_sound(snd_E_B, 1, false);
							
						}else{
							//Cast spell
							if (ds_psi[| selectedspell] <= 7){
								if (ds_psi[| selectedspell] == 6) herototaldamage = irandom_range(-10, -15);
								else if (ds_psi[| selectedspell] == 7) herototaldamage = irandom_range(15, 30);
								
								ga_party3[herotocommand.index, 4] -= 5;
								scr_eggbossdamage2(herototaldamage, optiontarget);
								...
								if (ds_psi[| selectedspell] == 8){
									...
									audio_play_sound(snd_psyche,1,false);
									ga_party3[herotocommand.index, 4] -= 10;
								}
								//SLEEP
								if (ds_psi[| selectedspell] == 9){
									...
									audio_play_sound(snd_psyche,1,false);
									ga_party3[herotocommand.index, 4] -= 15;
								}
							}
				...
				if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
					...
					if optiontarget.sprite_index == spr_btl_Edgar{
						...
						if (keyboard_check_pressed(global.key_down)){
							if ((selectedactor + 1) < ds_list_size(ds_target_boss)){
								selectedactor++;
							}else{
								selectedactor = 0;	
							}
							optiontarget = ds_target_boss[| selectedactor];
						}
						if keyboard_check_pressed(global.key_up){
							if ((selectedactor - 1) >= 0){
								selectedactor--;	
							}else{
								selectedactor = (ds_list_size(ds_target_boss) - 1);	
							}
							optiontarget = ds_target_boss[| selectedactor];
						}
					}else{
	...
	if (!playerturn){
		if actionstate == "INIT"{
			...
			//Recreate list
			ds_boss = ds_list_create();
			
			//Add valid heroes to list
			with(obj_EggBoss2){
				if number > 0 ds_list_add(other.ds_boss, id);	
			}
			
			ds_list_sort(ds_boss, true);
		...
		if actionstate == "READY"{
			...
			if (attacktimer >= timetillmonsterattacks){
				...
				activemonster = monstergroup;//ds_list_find_value(ds_boss, 0);
				ds_list_delete(ds_boss, 0);
				...
				maxmonsterdmg = ga_bosses[1, 5];
				damage = irandom_range(5, maxmonsterdmg);
				
				scr_eggbossdamage2(damage, victim);
				}
				attacktimer = 0;
			}
			
			if ds_list_size(ds_boss) <= 0{
				actionstate = "INIT";
				playerturn = true;
			}
		}
...
if state == "BATTLE OVER"{
	if battleovertext = "VICTORY!"
	{
		audio_stop_sound(mus_Edgar_Means_Business);	
		if !audio_is_playing(mus_Stats_and_Config)
		{
		audio_play_sound(mus_Stats_and_Config, 1, true, 50);
		}
	}
	if battleovertext = "YOU SNOOZE, YOU LOSE!"
	{
		audio_stop_sound(mus_Edgar_Means_Business);	
		if !audio_is_playing(mus_Try_Again)
		{
		audio_play_sound(mus_Try_Again, 1, true, 50);
		}
	...
	if keyboard_check(vk_space) && (timer >= (room_speed * 2)){
		load_game(2);
		with obj_BOSShero2{
			instance_destroy();
		}
		...
		if audio_is_playing(mus_Stats_and_Config) audio_stop_sound(mus_Stats_and_Config);
		if audio_is_playing(mus_Try_Again) audio_stop_sound(mus_Try_Again);
