...
for(var i = 0; i < array_length(a_menu); i++;){
	...
	if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
		draw_sprite_ext(spr_btlarrow, 0, optiontarget.x + (sprite_get_width(spr_btl_Edgar) / 24), optiontarget.y-64, 1, 1, 270, c_white, 1);	
...
numberofheroes = array_length(ga_party3)
uiX = BUFFER;
uiY = room_height - (numberofheroes * (fontsize + BUFFER));

for (var i = 0; i < numberofheroes; i++){
	draw_set_font(FontNumbers);
	draw_text(uiX + string_width("H") + (i*32), uiY-24, string(ga_party3[i, 2]) + string(" HP"));
	draw_text(uiX + string_width("H") + (i*32), uiY-12, string(ga_party3[i, 4]) + string(" PP"));
}
...
if state == "BATTLE OVER"{
	...
	draw_set_alpha(1);
	draw_text(room_width/2, room_height/8, string(battleovertext));
}