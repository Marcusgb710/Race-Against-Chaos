if (state == "INIT") {
	...
	ds_target_monstersPS = ds_list_create();
	
	with (obj_heroPS){
		instance_destroy();	
	}
	
	with (obj_monster_groupPS){
		instance_destroy();	
	}
	//Spawn party
	...
	for (var i = 0; i < partysize; i++){
		hero = instance_create_depth(heroX + (i*32), heroY, -100, obj_heroPS);
		hero.index = i;
		hero.dead = false;
		hero.stunned = 0;
		hero.isasleep = false;
	...
	for(var i = 0; i < totalmonstergroups; i++){
		monstergroup = instance_create_depth(a_monster_pos[i, 0], a_monster_pos[i, 1], -100, obj_monster_groupPS)	
		monstergroup.type = irandom_range(0,2);
		monstergroup.number = MAX_MONSTERS_PER_GROUP;
		monstergroup.dead = false;
		monstergroup.hp = ga_PSenemies[monstergroup.type, 1];
		monstergroup.stunned = 0;
		monstergroup.isasleep = false;
		ds_list_add(ds_target_monstersPS, monstergroup);
	}
	playerturn = true;
	actionstate = "INIT";
	psistate = "MAIN";
...
if (state == "READY"){
	if !audio_is_playing(mus_Battle_Against_A_Psyche_Enemy)
	{
	audio_play_sound(mus_Battle_Against_A_Psyche_Enemy, 1, true, 50);
	}
			...
			if (keyboard_check_pressed(vk_space)){
					...
					if menuselected == 0{
						if optionstate != "ATTACK"{
							...
							selectedactor = ds_list_find_index(ds_target_monstersPS, optiontarget)
							...
							herototaldamage = irandom_range(1, heromaxdamage-4);
							
							scr_damagePS(herototaldamage, optiontarget);
					...
					if menuselected == 1{
						if (optionstate != "PSI"){
							...
							if (ds_psi[| selectedspell] <= 7){
								...
								else if (ds_psi[| selectedspell] == 7) herototaldamage = irandom_range(25, 50);
				...
				if (optionstate == "ATTACK") || (psistate == "CHOOSE") || (optionstate == "CHOOSE"){
					...
					if optiontarget.sprite_index == spr_psmonsters{
						...
						if (keyboard_check_pressed(global.key_down)){
							if ((selectedactor + 1) < ds_list_size(ds_target_monstersPS)){
								selectedactor++;
							}else{
								selectedactor = 0;	
							}
							optiontarget = ds_target_monstersPS[| selectedactor];
						}
						if keyboard_check_pressed(global.key_up){
							if ((selectedactor - 1) >= 0){
								selectedactor--;	
							}else{
								selectedactor = (ds_list_size(ds_target_monstersPS) - 1);	
							}
							optiontarget = ds_target_monstersPS[| selectedactor];
						}
					}else{
	...
	if (!playerturn){
		if actionstate == "INIT"{
			...
			//Recreate list
			ds_monstersPS = ds_list_create();
			
			//Add valid heroes to list
			with(obj_monster_groupPS){
				if number > 0 ds_list_add(other.ds_monstersPS, id);	
			}
			
			ds_list_sort(ds_monstersPS, true);
		...
		if actionstate == "READY"{
			...
			if (attacktimer >= timetillmonsterattacks){
				...
				activemonster = ds_list_find_value(ds_monstersPS, 0);
				ds_list_delete(ds_monstersPS, 0);
				...
				if ga_PSenemies[activemonster.image_index, 4] > 15
				{
					roll = choose("ATT", "PSI")
					
					if roll == "PSI" && ga_PSenemies[activemonster.image_index, 10] == true 
					{
					
					randomvictim = irandom(ds_list_size(ds_target_heroes) - 1);
					victim = ds_target_heroes[| randomvictim];
					if ga_party[randomvictim, 4] >= 0
					{
					ga_party[randomvictim, 4] -= irandom_range(5, 25);
					ga_PSenemies[activemonster.image_index, 4] -= 25;	
					}else{
					scr_damagePS(-25, activemonster);
					}
					}else{
					roll = "ATT"	
					}
				...
				randomvictim = irandom(ds_list_size(ds_target_heroes) - 1);
				victim = ds_target_heroes[| randomvictim];
				maxmonsterdmg = ga_PSenemies[activemonster.image_index, 5];
				damage = irandom_range(maxmonsterdmg-10, maxmonsterdmg);
				
				scr_damagePS(damage, victim);	
			...
			if ds_list_size(ds_monstersPS) <= 0{
				actionstate = "INIT";
				playerturn = true;
			}
		}
...
if state == "BATTLE OVER"{
	...
	audio_stop_sound(mus_Battle_Against_A_Psyche_Enemy);
