...
function add_item(_inventory, _items, _item){
	...
	if(_inventory_size >= 10){show_debug_message("INVENTORY IS FULL CANNOT ADD ITEM") return"INVENTORY IS FULL CANNOT ADD ITEM"}
...
function remove_item(_inventory, _items, _item){
	var _inventory_size = array_length(_inventory)
	if(_inventory_size <= 0){show_debug_message("INVENTORY IS EMPTY CANNOT REMOVE ITEM") return"INVENTORY IS EMPTY CANNOT REMOVE ITEM"}
