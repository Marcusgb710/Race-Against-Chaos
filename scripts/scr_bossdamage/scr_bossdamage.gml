...
function scr_eggbossdamage(totaldamage, target){

//Check to see target

//Target is a monster
if (target.sprite_index == spr_btl_WeakEdgar){
	...
	//If every enemy is dead
		if target.number <= 0{
			...
			if (ds_list_size(ds_target_boss) == 0){
				obj_Eggbattle1.state = "BATTLE OVER";
				obj_Eggbattle1.battleovertext = "VICTORY!";
			}
		}else{
	...
	if target.sprite_index == spr_UIs{
		...
		ga_party1[target.index, 2] -= totaldamage;
		//If hero loses all hp, hero is dead
		if ga_party1[target.index, 2] <= 0{
			target.dead = true;	
			...
			if (ds_list_size(ds_target_heroes) == 0){
				obj_Eggbattle1.state = "BATTLE OVER";
				obj_Eggbattle1.battleovertext = "YOU SNOOZE, YOU LOSE!";
			}
		}
	  }
	}
