...
function scr_damageST(totaldamage, target){

//Check to see target

//Target is a monster
if (target.sprite_index == spr_stmonsters){
	...
	//If every enemy is dead
		if target.number <= 0{
			target.dead = true;	
			pos = ds_list_find_index(ds_target_monstersST, target);
			ds_list_delete(ds_target_monstersST, pos);
			
			if (ds_list_size(ds_target_monstersST) == 0){
				obj_battle_spawnerST.state = "BATTLE OVER";
				obj_battle_spawnerST.battleovertext = "VICTORY!";
			}
		}else{
			target.hp = ga_enemiesST[target.type, 1];	
		}
	}
}else{
	//Target is a hero
	if target.sprite_index == spr_UIs{
		...
		if ga_party[target.index, 2] <= 0{
			...
			if (ds_list_size(ds_target_heroes) == 0){
				obj_battle_spawnerST.state = "BATTLE OVER";
				obj_battle_spawnerST.battleovertext = "FAILURE.";
			}
		}
	  }
	}
